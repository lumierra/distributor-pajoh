...
const props = defineProps({
    openings: { type: Object, required: true },
...
const canCreate = computed(
    () => page.props.auth?.user?.is_superadmin || page.props.permissions?.['inventory.opening']?.create,
);
...
const statTiles = computed(() => {
    ...
    return [
        { label: 'Total', value: props.stats.total ?? 0, icon: PackagePlus },
...
<template>
    ...
    <AppLayout>
        <PageHeader
            title="Stok Awal (Opening Balance)"
            description="Input saldo awal stok yang sudah ada di gudang saat aplikasi mulai dipakai — tanpa PO."
            :icon="PackagePlus"
        >
            <template #actions>
                <Button
                ...
                >
                    <Link :href="route('openings.create')">
                        <Plus class="size-4" />
                        Buat Stok Awal
                    </Link>
                </Button>
        ...
        <section class="rounded-2xl bg-card/70 backdrop-blur-xl ring-1 ring-foreground/6 shadow-sm overflow-hidden">
            ...
            <div v-if="openings.data.length === 0" class="px-4 py-16">
                <div class="flex flex-col items-center gap-2 text-muted-foreground">
                    <PackagePlus class="size-7 opacity-40" />
                    <p class="text-sm">Belum ada stok awal.</p>
                </div>
            </div>
            ...
            <Table v-else class="border-t border-foreground/5">
                <TableHeader>
                    <TableRow class="[&>th]:text-[11px] [&>th]:font-semibold [&>th]:uppercase [&>th]:tracking-wider [&>th]:text-muted-foreground [&>th]:py-2.5 hover:bg-transparent">
                        <TableHead class="pl-4">No. Stok Awal</TableHead>
                    ...
                    >
                        <TableCell class="pl-4 py-2.5">
                            <div class="flex items-center gap-3">
                                ...
                                <Link :href="route('openings.show', o.id)" class="font-medium text-foreground font-mono hover:text-primary transition-colors">
                                    {{ o.opening_number }}
                                </Link>
                            </div>
                        </TableCell>
                        <TableCell class="text-xs text-muted-foreground">{{ formatDate(o.opening_date) }}</TableCell>
                        <TableCell class="text-right font-mono">{{ o.items_count }}</TableCell>
                        <TableCell>
                            <AdjustmentStatusBadge :status="o.status" />
                        </TableCell>
                            ...
                            <div class="flex justify-center">
                                <ActionGroup class="rounded-full">
                                    <ActionButton :icon="Eye" label="Detail" as-child tone="brand">
                                        <Link :href="route('openings.show', o.id)">
                                            <Eye class="w-4 h-4" />
                                        </Link>
                                    </ActionButton>
                                </ActionGroup>
            ...
            <div v-if="openings.data.length > 0" class="border-t border-foreground/5 px-4 py-3">
                <Pagination :meta="openings" />
            </div>
        </section>
    </AppLayout>
</template>
