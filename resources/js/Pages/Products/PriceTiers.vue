...
import { ArrowLeft, Layers, Loader2, Lock, Pencil, Plus, Trash2 } from '@lucide/vue';
...
function submit() {
    ...
    if (isEdit.value) {
        form.put(route('price-tiers.update', editing.value.id), opts);
    } else {
        form.post(route('price-tiers.store'), opts);
    }
}

function destroy(t) {
    if (! window.confirm(`Hapus tier ${t.name}?`)) return;
    useForm({}).delete(route('price-tiers.destroy', t.id), { preserveScroll: true });
}
</script>
...
<template>
    ...
    <AppLayout>
        <PageHeader
            title="Tier Harga"
            description="Master tier harga (GROSIR, ECERAN, MODERN_TRADE…). Tier system tidak bisa dihapus."
            :icon="Layers"
        >
            <template #actions>
            ...
            <Table>
                <TableHeader>
                    <TableRow class="[&>th]:text-[10px] [&>th]:font-semibold [&>th]:uppercase [&>th]:tracking-wider [&>th]:text-muted-foreground [&>th]:py-2.5">
                        ...
                        <TableHead class="text-right">Harga Aktif</TableHead>
                        <TableHead class="text-center">Status</TableHead>
                        <TableHead class="text-right pr-4">Aksi</TableHead>
                    </TableRow>
                </TableHeader>
                <TableBody class="text-sm">
                    <TableRow v-if="tiers.length === 0">
                    ...
                    >
                        <TableCell class="pl-4 py-2.5 font-mono text-xs">
                            <div class="flex items-center gap-1.5">
                                {{ t.code }}
                                <Lock v-if="t.is_system" class="size-3 text-muted-foreground" />
                            </div>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell class="text-right tabular-nums">{{ t.prices_count }}</TableCell>
            ...
            <DialogContent class="sm:max-w-[480px] p-0 overflow-hidden">
                    ...
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <DialogTitle class="text-base font-bold tracking-tight">
                                {{ isEdit ? `Edit Tier — ${editing.name}` : 'Tambah Tier Harga' }}
                            </DialogTitle>
                ...
                <form class="px-5 py-4 space-y-3.5" @submit.prevent="submit">
                    <div v-if="!isEdit" class="space-y-1">
                        ...
                        <Input v-model="form.code" required class="h-9 font-mono uppercase" placeholder="GROSIR" />
