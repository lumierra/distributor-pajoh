...
function submit() {
    ...
    if (isEdit.value) {
        form.put(route('product-categories.update', editing.value.id), opts);
    } else {
        form.post(route('product-categories.store'), opts);
    }
}

function destroy(c) {
    if (! window.confirm(`Hapus kategori ${c.name}?`)) return;
    useForm({}).delete(route('product-categories.destroy', c.id), {
        preserveScroll: true,
    });
}
</script>
...
<template>
    ...
    <AppLayout>
        <PageHeader
            ...
            description="Master kategori produk. Code dipakai oleh NumberingService untuk SKU."
            :icon="Tags"
        >
            <template #actions>
            ...
            <Table>
                ...
                <TableBody class="text-sm">
                    ...
                    >
                        ...
                        <TableCell class="text-right tabular-nums">{{ c.products_count }}</TableCell>
                        ...
                        <TableCell class="pr-4">
                            <div class="flex justify-end">
                                <ActionGroup>
                                    ...
                                    <ActionButton :icon="Trash2" label="Hapus" tone="red" @click="destroy(c)" />
                                </ActionGroup>
                            </div>
                        </TableCell>
                    </TableRow>
            ...
            <DialogContent class="sm:max-w-[480px] p-0 overflow-hidden">
                ...
                <form class="px-5 py-4 space-y-3.5" @submit.prevent="submit">
                    <div v-if="!isEdit" class="space-y-1">
                        ...
                        <Input v-model="form.code" required class="h-9 font-mono uppercase" placeholder="MIE" />
