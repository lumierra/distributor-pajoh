...
defineProps({
    ...
    /** 'neutral' (default, dipakai tab konten) | 'brand' (tint merah, dipakai filter list) */
    tone: {
        type: String,
        default: 'neutral',
        validator: (v) => ['neutral', 'brand'].includes(v),
    },
});
...
<template>
    <div
        class="inline-flex items-center gap-1 p-1 rounded-full bg-card ring-1 ring-foreground/10 shadow-xs"
    >
        <button
            v-for="t in tabs"
            :key="t.value"
